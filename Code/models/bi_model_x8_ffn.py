...
class PatchEmbed(nn.Module):
    ...
    def __init__(
        self, img_size=224, patch_size=1, in_chans=64, embed_dim=192, norm_layer=None
    ):
        super().__init__()
...
class bi_model_x8(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.Fconv_1 = nn.Sequential(
            nn.Conv2d(1, self.de_features, kernel_size=5, stride=1, padding=2),
            # nn.BatchNorm2d(self.de_features),
            nn.Conv2d(
                self.de_features, self.de_features, kernel_size=3, stride=1, padding=1
            ),
        )
        self.Fconv = nn.Sequential(
            nn.Conv2d(3, self.coex_features, kernel_size=5, stride=1, padding=2),
        ...
        # Depth-wise conv
        # self.dconv = Dconvbasic(embed_dim=embed_dim)
        """
        self.dconv = nn.Sequential(
            nn.Conv2d(64,out_channels=self.embed_dim,kernel_size=3,stride=1,padding=1,groups=embed_dim)
            nn.PReLU()
        )
        """
        ...
        self.patch_embed_y = PatchEmbed(
            img_size=img_size_y,
            patch_size=patch_size,
            in_chans=self.coex_features,
            embed_dim=embed_dim_y,
            norm_layer=norm_layer if self.patch_norm else None,
        )
        ...
        if self.ape:
            ...
            self.absolute_pos_embed_y = nn.Parameter(
                torch.zeros(1, num_patches_y, embed_dim)
            )
            trunc_normal_(self.absolute_pos_embed_y, std=0.02)
        ...
        dpr = [
            x.item() for x in torch.linspace(0, drop_path_rate, sum(self.down_depths))
        ]  # stochastic depth decay rule
        self.downsample_layers = nn.ModuleList()
        for i_layer in range(self.down_num):
            layer = BasicLayer(
                dim=int(embed_dim_y * 2 ** i_layer),
                input_resolution=(
                    patches_resolution_y[0] // (2 ** i_layer),
                ...
                drop_path=dpr[
                    sum(self.down_depths[:i_layer]) : sum(
                        self.down_depths[: i_layer + 1]
                    )
                ],
                norm_layer=norm_layer,
        ...
        self.guidedlayer0 = BasicLayer(
            dim=int(embed_dim_y * 2 ** self.down_num),
        ...
        # CrossAttention
        self.crosslayer_d0 = CrossTransformerblock(
            dim=int(embed_dim),
        ...
        self.crosslayer_d1 = CrossTransformerblock(
            dim=int(embed_dim),
            y_dim=int(embed_dim_y * 2 ** self.down_num),
            input_resolution=(patches_resolution[0], patches_resolution[1]),
            num_heads=num_heads[1],
        ...
        self.crosslayer_d2 = CrossTransformerblock(
            dim=int(embed_dim),
            y_dim=int(embed_dim_y * 2 ** self.down_num),
            input_resolution=(patches_resolution[0], patches_resolution[1]),
            num_heads=num_heads[1],
        ...
        self.crosslayer_c0 = CrossTransformerblock(
            dim=int(embed_dim_y * 2 ** self.down_num),
            y_dim=int(embed_dim),
            input_resolution=(patches_resolution[0], patches_resolution[1]),
            num_heads=num_heads[1],
        ...
        self.crosslayer_c1 = CrossTransformerblock(
            dim=int(embed_dim_y * 2 ** self.down_num),
            y_dim=int(embed_dim),
            input_resolution=(patches_resolution[0], patches_resolution[1]),
            num_heads=num_heads[1],
        ...
        self.crosslayer_c2 = CrossTransformerblock(
            dim=int(embed_dim_y * 2 ** self.down_num),
            y_dim=int(embed_dim),
            input_resolution=(patches_resolution[0], patches_resolution[1]),
            num_heads=num_heads[1],
        ...
        self.before_upconv = nn.Sequential(
            nn.Conv2d(
        ...
        self.before_upconv_c = nn.Sequential(
            nn.Conv2d(
                in_channels=int(embed_dim_y * 2 ** self.down_num),
                out_channels=64,
                kernel_size=1,
                stride=1,
                padding=0,
    ...
    def forward(self, x, y):  #  x is depth, y is color
        ...
        # x = x.transpose(1, 2).view(B, 4 * self.embed_dim, x_size[0], x_size[1])
        ...
        # x_hr = F.interpolate(x_lr, scale_factor=4, mode='bicubic').type(torch.cuda.FloatTensor)

...
class bi_model_x16(nn.Module):
    ...
    def __init__(self, args ,img_size=16, img_size_y=256,scale_factor=16,patch_size=1, in_chans=1, out_chans=1,in_chans_y=3,
                 embed_dim=64,de_features=64,embed_dim_y=8,coex_features=8, up_heads=[1,1,1,2],num_heads=[4,2],depths=[2,2,2,2], guid_depths=[2,2,2,2], 
        ...
        self.depthlayer3 = BasicLayer(dim=int(embed_dim),
                               ...
                               norm_layer=norm_layer,
                               use_checkpoint=use_checkpoint)
        # self.depth_dconv3 = Dconvbasic(embed_dim=int(embed_dim))
        ...
        # self.before_upconv = nn.Sequential(nn.Conv2d(in_channels=4 * embed_dim, out_channels=4 * embed_dim, kernel_size=3,stride=1,padding=1,bias=True),
        #                                     nn.Conv2d(4 * embed_dim,64,kernel_size=1,stride=1,padding=0,bias=False),
        #                                     nn.PReLU()
        #                                     )                                    

...
def add_noise(x, k=1, sigma=5, inv=True):                                                                                                                                                                 
    # x: [H, W, 1]                                                                                                                                                                                          
...
class Trainer(object):
    ...
    def train_step(self, data):
        ...
        input = data["lr_depth"]
        # a = time.time()
        depth_sr, cdepth_sr, fudepth_sr = self.model(input, color)
        # b = time.time()
        # self.sum_t += (b-a)
        # print("loss--", gt.shape, pred.shape)
        loss_d = self.objective(depth_sr, gt)
        loss_c = self.objective(cdepth_sr, gt)
        loss_fu = self.objective(fudepth_sr, gt)
        loss = 0.5 * loss_d + 0.5 * loss_c + loss_fu
        ...
        return depth_sr, cdepth_sr, fudepth_sr, gt, loss
    ...
    def eval_step(self, data):
        ...
        input = data["lr_depth"]
        color = data["hr_image"]
        _, c, depth_h_old, depth_w_old = input.size()
        _, _, rgb_h, rgb_w = color.size()
        ###_, c, depth_h_old, depth_w_old = input.shape
        ###pad = torch.zeros([1, c, depth_h_old, depth_w_old], dtype=torch.float32)
        h_pad = (
            depth_h_old // self.args.window_size + 1
        ) * self.args.window_size - depth_h_old
        w_pad = (
            depth_w_old // self.args.window_size + 1
        ) * self.args.window_size - depth_w_old

        input = torch.cat([input, torch.flip(input, [2])], 2)[
            :, :, : depth_h_old + h_pad, :
        ]
        input = torch.cat([input, torch.flip(input, [3])], 3)[
            :, :, :, : depth_w_old + w_pad
        ]
        ###input = torch.cat([input, pad], 2)[:, :, :depth_h_old + h_pad, :]
        ###input = torch.cat([input, pad], 3)[:, :, :, :depth_w_old + w_pad]
        color = torch.cat([color, torch.flip(color, [2])], 2)[
            :, :, : rgb_h + self.args.scale * h_pad, :
        ]
        color = torch.cat([color, torch.flip(color, [3])], 3)[
            :, :, :, : rgb_w + self.args.scale * w_pad
        ]
    ...
    def test_step(self, data):
        ...
        return depth_sr, cdepth_sr, fudepth_sr, gt, dtime, input

    ### ------------------------------

    def train(self, train_loader, valid_loader, max_epochs, AR_epoch=None):
    ...
    def test(self, loader, save_path=None):
        if save_path is None:
            save_path_d = os.path.join(
                self.workspace,
                "results_d",
                f"{self.name}_{self.args.dataset}_{self.args.scale}",
            )
            save_path_c = os.path.join(
                self.workspace,
                "results_c",
                f"{self.name}_{self.args.dataset}_{self.args.scale}",
            )
            save_path_fu = os.path.join(
                self.workspace,
                "results_fu",
                f"{self.name}_{self.args.dataset}_{self.args.scale}",
            )
            ...
            csv_path = os.path.join(
                self.workspace,
                f"{self.name}_{self.args.dataset}_{self.args.scale}.csv",
            )
        ...
        with torch.no_grad():
            ...
            for data in loader:
                ...
                depth_sr, cdepth_sr, fudepth_sr, gt, dtime, input = self.test_step(data)
                all_time += dtime
                # print(fudepth_sr)
                for metric in self.metrics:
                    # print(metric.V)
                    metric.update(data, fudepth_sr, gt, csv_writer)
                ...
                for b in range(fudepth_sr.shape[0]):
                    ...
                    if not isinstance(idx, str):
                        idx = str(idx.item())
                    # pred = preds[b][0]
                    # depth_sr = depth_sr[b].transpose(1, 2, 0)
                    # cdepth_sr = cdepth_sr[b].transpose(1, 2, 0)
                    # fudepth_sr = fudepth_sr[b].transpose(1, 2, 0)
                    # gt = gt[b].transpose(1, 2, 0)
                    # input = input[b].transpose(1, 2, 0)
                    ...
                    plt.imsave(os.path.join(save_path_gt, self.name + idx + ".png"),gt,cmap="gray")
                    ####other
                    ...
                    # cv2.imwrite(os.path.join(save_path_gt, self.name + idx + ".png"),gt)


                pbar.update(loader.batch_size)
            avg_rmse = metric.V / len(loader)
        self.log(f"==> Finished Test.")
        self.log(metric.report())
        print("avg_rmse:" , avg_rmse)
        print(
            "total time, pics, average time:",
            all_time,
            len(loader),
            all_time / len(loader),
        )
    ...
    def train_one_epoch(self, loader):
        ...
        for data in loader:
            ...
            data = self.prepare_data(data)
            # 前向并计算loss
            depth_sr, cdepth_sr, fudepth_sr, truths, loss = self.train_step(data)
            # loss *= 100
    ...
    def evaluate_one_epoch(self, loader):
        ...
        with torch.no_grad():
            ...
            for data in loader:
                ...
                data = self.prepare_data(data)
                # a = time.time()
                depth_sr, cdepth_sr, fudepth_sr, truths, loss = self.eval_step(data)
                # b = time.time()
                # self.sum_t += b-a
